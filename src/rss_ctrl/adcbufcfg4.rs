...
Number of chirps to be stored in Ping buffer. This register should be programmed with one less than the actual number needed. This is used when data is written to Pong Memory. The value written to this field should be the same as that configured for Pong"]
...
#[doc = "Field `ADCBUFNUMCHRPPONG` reader - 25:21\\]
Number of chirps to be stored in Pong buffer. This register should be programmed with one less than the actual number needed. This is used when data is written to Pong Memory. The value written to this field should be the same as that configured for Ping"]
pub type AdcbufnumchrppongR = crate::FieldReader;
#[doc = "Field `ADCBUFNUMCHRPPONG` writer - 25:21\\]
Number of chirps to be stored in Pong buffer. This register should be programmed with one less than the actual number needed. This is used when data is written to Pong Memory. The value written to this field should be the same as that configured for Ping"]
pub type AdcbufnumchrppongW<'a, REG> = crate::FieldWriter<'a, REG, 5>;
...
Number of chirps to be stored in Pong buffer. This register should be programmed with one less than the actual number needed. This is used when data is written to Pong Memory. The value written to this field should be the same as that configured for Ping"]
    #[inline(always)]
    pub fn adcbufnumchrppong(&self) -> AdcbufnumchrppongR {
        AdcbufnumchrppongR::new(((self.bits >> 21) & 0x1f) as u8)
    }
    #[doc = "Bit 30 - 30:30\\]
TI Internal Feature 0 --> Delay Interrupt Gen and Ping/Pong toggle together based on cfg_interrupt_gen_delay, 1 --> Delay only Interrupt Gen based on cfg_interrupt_gen_delay. But toggle Ping/Pong select signal as soon as the write is complete."]
    #[inline(always)]
...
Number of chirps to be stored in Pong buffer. This register should be programmed with one less than the actual number needed. This is used when data is written to Pong Memory. The value written to this field should be the same as that configured for Ping"]
    #[inline(always)]
    #[must_use]
    pub fn adcbufnumchrppong(&mut self) -> AdcbufnumchrppongW<Adcbufcfg4Spec> {
        AdcbufnumchrppongW::new(self, 21)
    }
    #[doc = "Bit 30 - 30:30\\]
TI Internal Feature 0 --> Delay Interrupt Gen and Ping/Pong toggle together based on cfg_interrupt_gen_delay, 1 --> Delay only Interrupt Gen based on cfg_interrupt_gen_delay. But toggle Ping/Pong select signal as soon as the write is complete."]
    #[inline(always)]
