...
External control used to assert the ATB signal AFVALIDS and drain any historical FIFO information on the bus"]
...
Return acknowledgement to CSETBFLUSHIN"]
...
When HIGH, indicates that trace acquisition is complete by ETB, that is, the trigger counter is at zero"]
...
When HIGH indicates that the ETB RAM has overflowed or wrapped around to address zero"]
